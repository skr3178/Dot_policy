...
"""The implementation of the Decoder-Only Transformer (DOT2) policy.
...
from lerobot.common.policies.dot2.configuration_dot2 import DOTConfig2
from lerobot.common.policies.normalize import Normalize, Unnormalize
from lerobot.common.policies.pretrained import PreTrainedPolicy


class DOT2(nn.Module):
    def __init__(self, config: DOTConfig):
        ...
        # Setup transformer decoder # Boil code from pytorch
        dec_layer = nn.TransformerDecoderLayer(
            d_model=self.config.dim_model,  # the number of expected features in the input
            nhead=self.config.n_heads,  # the number of heads in the multi-head attention models
            dim_feedforward=self.config.dim_feedforward,  # the dimension of the feedforward network model
            ...
            norm_first=self.config.pre_norm, # if True, layer norm is done prior to self attention, multihead attention and feedforward operations, respectively.
        )

        decoder_norm = nn.LayerNorm(self.config.dim_model)
        self.decoder = nn.TransformerDecoder( # Creates a stack of N decoder layers and adds layer norm in the end
        ...
        # decoder_pos created: [30, 128]
        decoder_pos = create_sinusoidal_pos_embedding( # return pos_embeddings of shape [train_horizon + lookback_obs_steps, dim_model]
            config.train_horizon + config.lookback_obs_steps, config.dim_model # [20 + 10, 128]
        )
        # filtered into shape [22, 128]. Why?
        decoder_pos = torch.cat(
            [
                decoder_pos[:1],
                decoder_pos[-config.train_horizon - config.n_obs_steps + 2 :], # config.train_horizon:20, config.n_steps: 3
            ],
            dim=0,
        )
        ...
        # Fixed positional encoding (like the sinusoidal one in the original Transformer paper) uses a deterministic formula that maps each position to a vector.
        # The model can learn position representations tailored to the dataset, If your data doesn't have uniformly spaced positions Tasks where position interacts with meaning in a non-linear way

        self.inputs_pos_emb = nn.Parameter(torch.empty(1, self.n_features, self.config.dim_model))

        # fills in the input tensor from values drawn from uniform distribution U[a, b], where a: lower bound, b: upper bound of uni dist.
        nn.init.uniform_(
            self.inputs_pos_emb,
    ...
    def _process_inputs(self, batch):
        ...
        for state in self.projections_names: # proj_names: [images, state]
            batch_state = self.obs_mapping[state] # ['observation.images', 'observation.state']
            if batch_state in batch:
                bs, n_obs, *obs_shape = batch[batch_state].shape # [bs, n_obs, *obs_shape: 24, 3, 2]
...
class DOTPolicy2(PreTrainedPolicy):
    name = "dot2"
    config_class = DOTConfig2
    ...
    def __init__(
        ...
        # overwriting boolean flag that allows you to override the dataset statistics used for data normalization
        ...
        # action_weights: Used during inference for temporal smoothing/chunking of actions
        # loss_weights: Used during training to prioritize learning near-term vs far-term actions
        # Both use exponential weighting but serve different purposes:
        # Action weights ensure smooth robot control
        # Loss weights ensure effective learning priorities

...
def create_resnet_transformer_config():
    """Create a hardcoded configuration for ResNet transformer policy training."""
    from lerobot.common.envs.configs import PushtEnv
    from lerobot.common.policies.resnet_transformer import ResNetTransformerConfig
    ...
    offline_config = OfflineConfig(steps=100000)
    
    # Create online training config (disabled)
    online_config = OnlineConfig(steps=0)
    
    # Create evaluation config
    eval_config = EvalConfig(n_episodes=10, batch_size=4)
    
    # Create wandb config
    wandb_config = WandBConfig(enable=False)
    ...
    scheduler_config = CosineAnnealingSchedulerConfig(
        min_lr=1.0e-4,
        T_max=300000
    )
    ...
    config = TrainPipelineConfig(
        ...
        use_policy_training_preset=False  # Use our custom optimizer/scheduler instead of policy defaults
    )
...
def train():
    """Main training function with hardcoded ResNet transformer configuration."""
    # Create configuration programmatically
    cfg = create_resnet_transformer_config()

...
"""
This file contains a ResNet + Transformer model that can be integrated with the LeRobot training pipeline.

To use this model with the existing train.py:

1. Import the model and config:
   from resnet_adap_lerobot_2 import ResNetTransformerDecoder, ResNetTransformerConfig

2. Use it in your training config:
...
3. The model will automatically work with LeRobot's training pipeline, including:
   - Automatic optimizer and scheduler configuration
   - Data normalization
   - Checkpointing and model saving
   - Integration with wandb logging

The model inherits from PreTrainedPolicy, so it has all the required methods:
- forward(): Computes loss for training
- select_action(): Returns actions for inference
- get_optim_params(): Returns optimizer parameters
- reset(): Called when environment resets
"""
...
class ResNetTransformerDecoder(PreTrainedPolicy):
    ...
    def __init__(
        self,
        config: ResNetTransformerConfig,
    ...
    def reset(self):
        ...
        # No caches to clear for this model
        pass
    
    def forward(self, batch: dict[str, torch.Tensor]) -> dict:
        """Run the batch through the model and compute the loss for training or validation.
        
        Returns a dictionary with "loss" and potentially other information.
        """
        ...
        # Forward pass
        predicted_actions = self._forward_internal(images, states)
    ...
    def _forward_internal(self, images: torch.Tensor, states: torch.Tensor) -> torch.Tensor:
        """Internal forward pass for action prediction."""
        batch_size = images.size(0)
        
        # Check if normalizers are initialized
        if not self._normalizers_initialized:
            raise RuntimeError("Normalizers not initialized. Call _initialize_normalizers() first.")
...
def create_model(
    ...
    Returns:
        Configured model
    """
    model = ResNetTransformerDecoder(config, dataset_stats)
    return model
...
if __name__ == "__main__":
    print("="*50)
    print("Running full training example...")
    print("="*50)
    ...
    for batch in data_adapter.train_loader:
    ...
    images_flat = all_images.permute(0, 2, 3, 1).reshape(-1, channels)
    
    img_mean = images_flat.mean(dim=0, keepdim=True).unsqueeze(-1).unsqueeze(-1)
    img_std = images_flat.std(dim=0, keepdim=True).unsqueeze(-1).unsqueeze(-1)
    img_min_values, _ = images_flat.min(dim=0, keepdim=True)
    img_min = img_min_values.unsqueeze(-1).unsqueeze(-1)
    img_max_values, _ = images_flat.max(dim=0, keepdim=True)
    img_max = img_max_values.unsqueeze(-1).unsqueeze(-1)
    ...
    dataset_stats = {
        ...
        "observation.state": {
            ...
            "min": all_states.min(dim=0)[0],
            "max": all_states.max(dim=0)[0],
        },
        "action": {
    ...
    # Create model using the new create_model function
    model = create_model(config, dataset_stats)
    ...
    # Test model with LeRobot batch format
    test_batch = {
        "observation.image": images,
        "observation.state": states,
        "action": actions
    ...
    with torch.no_grad():
        ...
        predicted_actions = model.select_action(test_batch)
